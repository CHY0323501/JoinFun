        ...
        public ActionResult ForgetPwd(string account, string email)
        {
            ...
            if (acc != null)
            {
                if (acc.Account == account && db.Member.Where(m => m.memId == db.Acc_Pass.Where(c => c.Account == account).FirstOrDefault().memId).FirstOrDefault().Email == email)
                {
                    ...
                    mes.SendEmail(mailList, "JoinFun重置密碼通知", "<img src='https://i.ibb.co/dcBqtJk/img.png' style='width:25 %'><h2>親愛的會員:</h2></br><h2>請點擊下面連結來重置您的密碼!</h2></br><a href='http://10.10.3.105/Register/RemakePwd?email_ID=" + getEmailID + "'><h2>重置密碼請點擊</h2></a></br>");
